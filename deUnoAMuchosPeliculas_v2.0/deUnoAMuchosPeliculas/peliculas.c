...
void mostrarPelicilasConEstudio(ePelicula listaPeliculas[] , int tampeli, eEstudio listaEstudios[], int tamest)
{
    int i;
    int j;
    for(i=0; i<tampeli; i++)
    {
        if(listaPeliculas[i].estado==OCUPADO) {
            ...
            for(j=0; j<tamest; j++)
            {
                if( listaPeliculas[i].idEstudio == listaEstudios[j].id && listaEstudios[j].estado == OCUPADO)
                {
